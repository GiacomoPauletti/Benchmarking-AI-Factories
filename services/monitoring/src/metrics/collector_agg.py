...
class CollectorAgg:
    ...
    def collect_window(self, start_iso: str, end_iso: str) -> Dict[str, Any]:
        """
        Pull metrics for vLLM and Qdrant services.
        If a metric doesn't exist, we return None.
        """
        # vLLM Throughput (requests per second)
        vllm_throughput = self.query_range(
            'sum(rate(vllm:request_success_total[1m]))',
            start_iso, end_iso, step="15s"
        )
        throughput = statistics.mean([v for _, v in vllm_throughput]) if vllm_throughput else None

        # CPU util (node_exporter - if available)
        cpu_series = self.query_range(
            '1 - avg(rate(node_cpu_seconds_total{mode="idle"}[1m]))',
            start_iso, end_iso, step="15s"
        )
        cpu_util = 100.0 * statistics.mean([v for _, v in cpu_series]) if cpu_series else None

        # GPU util (vLLM GPU cache usage or DCGM)
        gpu_vllm = self.query_range(
            'avg(vllm:gpu_cache_usage_perc)',
            start_iso, end_iso, step="15s"
        )
        if gpu_vllm:
            gpu_util = statistics.mean([v for _, v in gpu_vllm])
        else:
            # Fallback to DCGM if available
            gpu_dcgm = self.query_range('avg(DCGM_FI_DEV_GPU_UTIL)', start_iso, end_iso, step="15s")
            gpu_util = statistics.mean([v for _, v in gpu_dcgm]) if gpu_dcgm else None

        # vLLM latency percentiles (from histogram)
        lat_p50_series = self.query_range(
            'histogram_quantile(0.50, sum by (le) (rate(vllm:e2e_request_latency_seconds_bucket[1m])))',
            start_iso, end_iso, step="15s"
        )
        lat_p50 = statistics.mean([v * 1000.0 for _, v in lat_p50_series]) if lat_p50_series else None

        lat_p95_series = self.query_range(
            'histogram_quantile(0.95, sum by (le) (rate(vllm:e2e_request_latency_seconds_bucket[1m])))',
            start_iso, end_iso, step="15s"
        )
        lat_p95 = statistics.mean([v * 1000.0 for _, v in lat_p95_series]) if lat_p95_series else None

        lat_p99_series = self.query_range(
            'histogram_quantile(0.99, sum by (le) (rate(vllm:e2e_request_latency_seconds_bucket[1m])))',
            start_iso, end_iso, step="15s"
        )
        lat_p99 = statistics.mean([v * 1000.0 for _, v in lat_p99_series]) if lat_p99_series else None
