...
class SSHManager:
    ...
    def ensure_remote_directories(self, remote_base_path: str):
        ...
        Creates base directory and subdirectories for recipes and logs if they don't exist.
        
        Args:
            remote_base_path: Base path on MeluXina (e.g., ~/ai-factory-benchmarks)
        """
        self.logger.info(f"Ensuring remote directories exist at {remote_base_path}")
        
        # Create base directory and all required subdirectories
        # mkdir -p ensures parent directories are created if needed
        dirs_to_create = [
            remote_base_path,  # Ensure base directory exists first
